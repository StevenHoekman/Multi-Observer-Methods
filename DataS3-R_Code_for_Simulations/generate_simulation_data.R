...
# Function for generating simulated survey data, version 1.01
# Steven T. Hoekman, Wild Ginger Consulting, PO Box 182 Langley, WA 98260, steven.hoekman@protonmail.com

# R computer code for generating simulated survey data for multi-observation method (MOM) and single-observation method (SOM) models. Accepts user-specified inputs for simulation analyses, returns list with formatted simulated survey data, (if applicable) tables of key values for observed groups and binned covariate values, and (if applicable) true values for true species probabilities and classification probabilities. 
...
# This function accepts input of simulation profiles 'sim_profiles', with simulation inputs for each distinct model on a separate row. To generate simulated true survey data (true species states (B) for individuals in each group), random samples are drawn from appropriate probability distributions using true parameter values for: 

# (1) true species probabilities (psi)
# (2) mean group sizes (g) for each true species
# (3) heterogeneous group parameters (pi and rho) for each species pair
...
# (1) Data frame 'survey_data' with simulated survey data consisting of observed groups for each survey observer, 'id' with integer value specifying simulation repetition, 'group' with group size, and a 'count' of identical observation histories. Included if applicable are values of predictive covariates and numeric key values for unique observed groups and covariate values. 
# (2) Optional data frame 'observed_group_key' with unique key values for unique observed groups
# (3) Optional data frame 'psi_key' with unique key values for unique values of a covariate predicting true species probabilities (psi) 
# (4) Optional vectors in 'mean_probability' with overall mean true values for true species probabilities and classification probabilities across values of predictive covariates
...
reps <- as.integer(sim$reps) # Number of simulation repetitions
...
# Surveys with covariate(s) predicting true species probability (psi)
  ...
  if (any(mix > 0)) {
    if (mx.model == "constant") {
      ...
      # Uses "encounter" model for group-level group probabilities (described in the companion article in Appendix S3: Eqs. S2, S3)
      if (B == 2) {
        # True species states = 2
...
if (sim$Model == "M" | sim$Model == "M.theta.p" | sim$Model == "mpX" | sim$Model == "M.psi") {
    ...
    for (b in 1:B) {
      for (obs in i:sum(O_ps)) {
        ...
        # For observer 'obs', generate 'r' observed groups and place in columns 'y'. Observations are randomly for each individual in a true group by random draws from a multinomial distribution with true classification probabilities 'theta.t'
...
# Add 'id' indexing simulation repetitions
